...
USERNAME_AVAIBILITY=$($PSQL "SELECT username FROM players WHERE username = '$USERNAME'")
GAMES_PLAYED=$($PSQL "SELECT COUNT (*) FROM players INNER JOIN games USING (user_id) WHERE username = '$USERNAME'")
BEST_GAME=$($PSQL "SELECT MIN(guess_number) FROM players INNER JOIN games USING (user_id) WHERE username = '$USERNAME'")
if [[ -z $USERNAME_AVAIBILITY ]]
then
